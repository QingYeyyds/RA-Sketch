#include <iostream>
#include <random>
#include <chrono>
#include <unordered_map>

using namespace std;

#define PRO 0.05
#define MAX_INSERT_PACKAGE 25000000
#define THRE 250

int TOT_FLOWS = 0;
int HEAVY_FLOWS = 0;

int epoch_sum = 0;

void load_data() {
    unordered_map<uint32_t, int> ground_truth;
    ground_truth.clear();

    const char *filename= "dataset";
    FILE *pf = fopen(filename, "rb");
    if (!pf) {
        cerr << filename << " not found." << endl;
        exit(-1);
    }

    char ip[4];
    int ret = 0;
    while(true){
        size_t rsize;
        rsize = fread(ip, 1, 4, pf);
        if(rsize != 4) break;
        uint32_t key = *(uint32_t *) ip;
        ground_truth[key]++;
        ret++;
        if (ret == MAX_INSERT_PACKAGE){
            cout <<"MAX_INSERT_PACKAGE"<< endl;
            break;
        }
    }
    fclose(pf);

    TOT_FLOWS = ground_truth.size();
    for(auto i:ground_truth)
        if(i.second >= THRE)
            HEAVY_FLOWS += 1;

    cout<<ret<<" "<<TOT_FLOWS<<" "<<HEAVY_FLOWS<<endl;
}

double bloom_fp(int k, int w){
    epoch_sum++;
    return pow(1 - exp(-1.0 * TOT_FLOWS/w), k);
}

double elastic_rr(int w){
    epoch_sum++;
    double lambda = 1.0 * HEAVY_FLOWS / w;
    double rr = 0, pro = 1 - exp(-lambda);
    double temp;
    for (int i = 1; i < 7; i++){
        temp = exp(-lambda) * pow(lambda, i) / tgamma(i + 1);
        rr += i * temp;
        pro -= temp;
    }
    rr += 7 * pro;
    rr /= lambda;
    return rr;
}

double hg_rr(int w){
    epoch_sum++;
    double lambda = 1.0 * HEAVY_FLOWS / w;
    double rr = 0, pro = 1 - exp(-lambda);
    double temp;
    for (int i = 1; i <= 8; i++){
        temp = exp(-lambda) * pow(lambda, i) / tgamma(i + 1);
        rr += i * temp;
        pro -= temp;
    }
    rr += 7 * pro;
    rr /= lambda;
    return rr;
}

double hk_rr(int k, int w){
    epoch_sum++;
    double lambda = 1.0 * HEAVY_FLOWS / w;
    double rr = exp(-lambda) * lambda;
    rr /= lambda;
    rr = 1 - pow(1 - rr, k);
    return rr;
}

class Parm
{
public:
    int k;
    int w;
    double met;

    Parm( int k_init, int w_init, double met_init){
        k = k_init;
        w = w_init;
        met = met_init;
    }
};

tuple<int, int, double> search_bf(double fp){
    vector<Parm> vec;

    int res_k, res_w;
    double res_fp = 0, cur_fp;

    for(int k=1; k<=3; k++){
        res_k = 0, res_w = 0;
        int bucket_num = TOT_FLOWS / pow(fp,1.0/k);
        cur_fp = bloom_fp(k, bucket_num);
        if(fabs(cur_fp - fp) <= PRO * fp){
            res_k = k;
            res_w = bucket_num;
            res_fp = cur_fp;
        }
        else if(cur_fp < fp){
            for(int w=bucket_num/2; w>0; w/=2){
                cur_fp = bloom_fp(k, w);
                if(fabs(cur_fp - fp) <= PRO * fp){
                    res_k = k;
                    res_w = w;
                    res_fp = cur_fp;
                    break;
                }
                else if(cur_fp > fp){
                    int low = w, high = w * 2, mid;
                    while(low < high){
                        mid = (low + high )/2;
                        cur_fp = bloom_fp(k, mid);
                        if(fabs(cur_fp - fp) <= PRO * fp) {
                            res_k = k;
                            res_w = mid;
                            res_fp = cur_fp;
                            break;
                        }
                        else if(cur_fp < fp)
                            high = mid - 1;
                        else
                            low = mid + 1;
                    }
                    break;
                }
            }
        }
        else{
            for(int w=bucket_num*2; ; w*=2){
                cur_fp = bloom_fp(k, w);
                if(fabs(cur_fp - fp) <= PRO * fp){
                    res_k = k;
                    res_w = w;
                    res_fp = cur_fp;
                    break;
                }
                else if(cur_fp < fp) {
                    int low = w / 2, high = w, mid;
                    while (low < high){
                        mid = (low + high) / 2;
                        cur_fp = bloom_fp(k, mid);
                        if (fabs(cur_fp - fp) <= PRO * fp) {
                            res_k = k;
                            res_w = mid;
                            res_fp = cur_fp;
                            break;
                        }
                        else if (cur_fp < fp)
                            high = mid - 1;
                        else if (cur_fp > fp)
                            low = mid + 1;
                    }
                    break;
                }
            }
        }
        if(res_k == 0 || res_w == 0)
            continue;
        vec.emplace_back(res_k,res_w,res_fp);
    }

    int min_mem = (1<<30), cur_mem;
    for(auto i:vec){
        cur_mem = i.k * i.w;
        if(cur_mem < min_mem){
            min_mem = cur_mem;
            res_k = i.k;
            res_w = i.w;
            res_fp = i.met;
        }
    }

    return make_tuple(res_k,res_w,res_fp);
}

tuple<int, double> search_elastic(double rr){
    int bucket_num = HEAVY_FLOWS * rr / 7;

    int res_w = 0;
    double res_rr = 0, cur_rr;

    cur_rr = elastic_rr( bucket_num);
    if(fabs(cur_rr - rr) <= PRO * rr){
        res_w = bucket_num;
        res_rr = cur_rr;
    }
    else if(cur_rr < rr){
        for(int w=bucket_num*2; ; w*=2){
            cur_rr = elastic_rr(w);
            if(fabs(cur_rr - rr) <= PRO * rr){
                res_w = w;
                res_rr = cur_rr;
                break;
            }
            else if(cur_rr > rr){
                int low = w/2, high = w, mid;
                while(low < high){
                    mid = (low + high )/2;
                    cur_rr = elastic_rr(mid);
                    if(fabs(cur_rr - rr) <= PRO * rr) {
                        res_w = mid;
                        res_rr = cur_rr;
                        break;
                    }
                    else if(cur_rr < rr)
                        low = mid + 1;
                    else
                        high = mid - 1;
                }
                break;
            }
        }
    }
    else{
        for(int w=bucket_num/2; w>0; w/=2){
            cur_rr = elastic_rr(w);
            if(fabs(cur_rr - rr) <= PRO * rr){
                res_w = w;
                res_rr = cur_rr;
                break;
            }
            else if(cur_rr < rr) {
                int low = w, high = w*2, mid;
                while (low < high){
                    mid = (low + high) / 2;
                    cur_rr = elastic_rr(mid);
                    if(fabs(cur_rr - rr) <= PRO * rr) {
                        res_w = mid;
                        res_rr = cur_rr;
                        break;
                    }
                    else if(cur_rr < rr)
                        low = mid + 1;
                    else
                        high = mid - 1;
                }
                break;
            }
        }
    }

    return make_tuple(res_w,res_rr);
}

tuple<int, double> search_hg(double rr){
    int bucket_num = HEAVY_FLOWS * rr / 8;

    int res_w = 0;
    double res_rr = 0, cur_rr;

    cur_rr = hg_rr( bucket_num);
    if(fabs(cur_rr - rr) <= PRO * rr){
        res_w = bucket_num;
        res_rr = cur_rr;
    }
    else if(cur_rr < rr){
        for(int w=bucket_num*2; ; w*=2){
            cur_rr = hg_rr(w);
            if(fabs(cur_rr - rr) <= PRO * rr){
                res_w = w;
                res_rr = cur_rr;
                break;
            }
            else if(cur_rr > rr){
                int low = w/2, high = w, mid;
                while(low < high){
                    mid = (low + high )/2;
                    cur_rr = hg_rr(mid);
                    if(fabs(cur_rr - rr) <= PRO * rr) {
                        res_w = mid;
                        res_rr = cur_rr;
                        break;
                    }
                    else if(cur_rr < rr)
                        low = mid + 1;
                    else
                        high = mid - 1;
                }
                break;
            }
        }
    }
    else{
        for(int w=bucket_num/2; w>0; w/=2){
            cur_rr = hg_rr(w);
            if(fabs(cur_rr - rr) <= PRO * rr){
                res_w = w;
                res_rr = cur_rr;
                break;
            }
            else if(cur_rr < rr) {
                int low = w, high = w*2, mid;
                while (low < high){
                    mid = (low + high) / 2;
                    cur_rr = hg_rr(mid);
                    if(fabs(cur_rr - rr) <= PRO * rr) {
                        res_w = mid;
                        res_rr = cur_rr;
                        break;
                    }
                    else if(cur_rr < rr)
                        low = mid + 1;
                    else
                        high = mid - 1;
                }
                break;
            }
        }
    }

    return make_tuple(res_w,res_rr);
}

tuple<int, int, double> search_hk(double rr){
    vector<Parm> vec;

    int res_k, res_w;
    double res_rr = 0, cur_rr;

    for(int k=1; k<=3; k++){
        res_k = 0, res_w = 0;
        int bucket_num = HEAVY_FLOWS * (1- pow(1-rr,1.0/k));
        cur_rr = hk_rr(k, bucket_num);
        if(fabs(cur_rr - rr) <= PRO * rr){
            res_k = k;
            res_w = bucket_num;
            res_rr = cur_rr;
        }
        else if(cur_rr < rr){
            for(int w=bucket_num*2; ; w*=2){
                cur_rr = hk_rr(k, w);
                if(fabs(cur_rr - rr) <= PRO * rr){
                    res_k = k;
                    res_w = w;
                    res_rr = cur_rr;
                    break;
                }
                else if(cur_rr > rr){
                    int low = w/2, high = w, mid;
                    while(low < high){
                        mid = (low + high )/2;
                        cur_rr = hk_rr(k, mid);
                        if(fabs(cur_rr - rr) <= PRO * rr) {
                            res_k = k;
                            res_w = mid;
                            res_rr = cur_rr;
                            break;
                        }
                        else if(cur_rr < rr)
                            low = mid + 1;
                        else
                            high = mid - 1;
                    }
                    break;
                }
            }
        }
        else{
            for(int w=bucket_num/2; w>0; w/=2){
                cur_rr = hk_rr(k, w);
                if(fabs(cur_rr - rr) <= PRO * rr){
                    res_k = k;
                    res_w = w;
                    res_rr = cur_rr;
                    break;
                }
                else if(cur_rr < rr) {
                    int low = w, high = w*2, mid;
                    while (low < high){
                        mid = (low + high) / 2;
                        cur_rr = hk_rr(k, mid);
                        if(fabs(cur_rr - rr) <= PRO * rr) {
                            res_k = k;
                            res_w = mid;
                            res_rr = cur_rr;
                            break;
                        }
                        else if(cur_rr < rr)
                            low = mid + 1;
                        else
                            high = mid - 1;
                    }
                    break;
                }
            }
        }
        if(res_k == 0 || res_w == 0)
            continue;
        vec.emplace_back(res_k,res_w,res_rr);
    }

    int min_mem = (1<<30), cur_mem;
    for(auto i:vec){
        cur_mem = i.k * i.w;
        if(cur_mem < min_mem){
            min_mem = cur_mem;
            res_k = i.k;
            res_w = i.w;
            res_rr = i.met;
        }
    }

    return make_tuple(res_k,res_w,res_rr);
}

int main() {
    load_data();
    int k , w;
    double res;
    auto beforeTime = chrono::steady_clock::now();
    tie( k , w, res) = search_bf(0.1);
//    tie( w, res) = search_elastic(0.9);
//    tie( w, res) = search_hg(0.9);
//    tie( k , w, res) = search_hk(0.9);
    auto afterTime = chrono::steady_clock::now();
    double duration_millsecond = chrono::duration<double, milli>(afterTime - beforeTime).count();
    cout << duration_millsecond << "ms"<<endl;

    cout<<"Epoch: "<<epoch_sum<<endl;

    cout<<"K: "<<k<<" W: "<<w<<" FP: "<<res<<endl;
//    cout<<"W: "<<w<<" RR: "<<res<<endl;
//    cout<<"W: "<<w<<" RR: "<<res<<endl;
//    cout<<"K: "<<k<<" W: "<<w<<" RR: "<<res<<endl;
    return 0;
}
